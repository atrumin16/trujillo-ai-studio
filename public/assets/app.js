(function() {
  ...
  function artifactKind(art) {
    ...
    var map = { html:'html', htm:'html', react:'html', jsx:'html', markdown:'markdown', md:'markdown', plaintext:'plaintext', text:'plaintext', txt:'plaintext', code:'code', javascript:'javascript', js:'javascript', python:'python', py:'python', json:'json', csv:'csv', svg:'svg', mermaid:'mermaid' };
  ...
  function syncArtifactActions() {
    ...
    if (openBtn) openBtn.hidden = !published;
    if (unBtn) unBtn.hidden = !published;
    if (pubBtn) pubBtn.textContent = published ? 'Actualizar' : 'Publicar';
  }
  function openArtifact(art) {
    ...
    if (title) title.textContent = art.title || 'Artifact';
    fillArtFormatSelect();
    var kind = artifactKind(art);
    art.lang = kind;
    var sel = document.getElementById('art-format');
    if (sel) sel.value = kind;
    var live = kind === 'html' || kind === 'svg' || kind === 'mermaid';
    if (preview && live) {
      preview.hidden = false;
      if (body) body.hidden = true;
      if (kind === 'mermaid') {
        preview.srcdoc = '<!DOCTYPE html><html><head><meta charset="UTF-8"><script src="https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.min.js"><\/script></head><body style="background:#080c14;margin:16px"><pre class="mermaid">' +
          String(art.content || '').replace(/</g, '&lt;') + '</pre><script>mermaid.initialize({startOnLoad:true,theme:"dark"});<\/script></body></html>';
      } else {
        preview.srcdoc = art.content || '';
      }
    } else {
      if (preview) { preview.hidden = true; preview.srcdoc = ''; }
      if (body) { body.hidden = false; body.textContent = art.content || ''; }
    }
    syncArtifactActions();
  ...
  async function publishArtifact() {
    ...
    if (!authToken) {
      alert('Inicia sesión para publicar en ai.trujillomingorance.com/artifact');
      return;
    }
    var suggested = currentArtifact.slug || slugifyArtifact(currentArtifact.title) || ('art-' + Date.now().toString(36));
    var slug = window.prompt('URL pública: ai.trujillomingorance.com/artifact/', suggested);
    if (!slug) return;
    ...
    try {
      var res = await fetch('/api/artifact', {
        ...
        body: JSON.stringify({
          ...
          lang: (document.getElementById('art-format') && document.getElementById('art-format').value) || currentArtifact.lang || 'markdown'
        })
      });
  ...
  async function unpublishArtifact() {
    ...
    if (!authToken) { alert('Inicia sesión para despublicar'); return; }
    if (!window.confirm('¿Despublicar /artifact/' + currentArtifact.slug + '?')) return;
    var btn = document.getElementById('art-unpublish-btn');
    if (btn) btn.disabled = true;
    try {
      var res = await fetch('/api/artifact/' + encodeURIComponent(currentArtifact.slug), {
        method: 'DELETE',
  ...
  async function openPublishedArtifact(slug) {
    if (!authToken || !slug) return;
    try {
      var res = await fetch('/api/artifact/' + encodeURIComponent(slug), {
        headers: { 'Authorization': 'Bearer ' + authToken }
      });
      var data = await res.json().catch(function() { return {}; });
  ...
  function renderArtifactsList(list, filterText) {
    ...
    if (!authToken) {
      ...
      need.textContent = 'Inicia sesión para ver tus artifacts publicados.';
    ...
    filtered.forEach(function(a) {
      ...
      slugEl.textContent = (a.lang || 'md') + ' · /artifact/' + a.slug;
      ...
      openPub.addEventListener('click', function(e) {
        e.stopPropagation();
        window.open(a.url || ('/artifact/' + a.slug), '_blank', 'noopener');
      });
      ...
      del.addEventListener('click', function(e) {
        e.stopPropagation();
        currentArtifact = { title: a.title, slug: a.slug, url: a.url, content: '', lang: a.lang };
      ...
      item.addEventListener('click', function() { openPublishedArtifact(a.slug); });
  ...
  function initApp() {
    ...
    if (artFormat) {
      artFormat.addEventListener('change', function() {
        if (!currentArtifact) return;
        currentArtifact.lang = artFormat.value;
