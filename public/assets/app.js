(function() {
  ...
  async function publishArtifactToGuides() {
    if (!currentArtifact) return;
    if (!authToken) {
      alert('Inicia sesión para publicar en guides.trujillomingorance.com');
      return;
    }
    var suggested = slugifyArtifact(currentArtifact.title) || ('art-' + Date.now().toString(36));
    var slug = window.prompt('URL pública: guides.trujillomingorance.com/a/', suggested);
    if (!slug) return;
    ...
    try {
      ...
      setTimeout(function() {
        var b = document.getElementById('art-publish-btn');
        if (b && b.textContent === 'Publicado') b.textContent = 'Publicar';
      }, 2500);
    }
  }
  ...
  function initApp() {
    ...
    if (artPublishBtn) artPublishBtn.addEventListener('click', publishArtifactToGuides);
